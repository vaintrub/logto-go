...
func TestOneTimeTokenWithJITOrgs(t *testing.T) {
	...
	createdOrg, err := testClient.CreateOrganization(ctx, fmt.Sprintf("JIT Org %d", time.Now().UnixNano()), "")
	...
	result, err := testClient.CreateOneTimeToken(ctx, email, 600, []string{orgID})
